// App.js starter code adapted from https://github.com/osu-cs340-ecampus/nodejs-starter-app
...
app.post('/add-guardian-ajax', function(req, res) 
{
    ...
    db.pool.query(query1, function(error, rows, fields){
        ...
        {
            //additional query to facilitate the ranks dropdown
...
app.post('/add-s-ajax', function(req, res){
    ...
    //INSERT function for sales, with NULLIF used to insert NULLable values
    ...
    (IFNULL((SELECT price FROM Consumables WHERE consumable_id = ${s_data.consumable_id}),0))), '${s_data.guardian_id}', NULLIF(${s_data.weapon_id},''), NULLIF(${s_data.cosmetic_id},''), NULLIF(${s_data.consumable_id},''));`;
...
app.post('/add-weapon-ajax', function(req, res){
    ...
    //NULLIF facilitates NULLable value
...
app.delete('/delete-gr-ajax', function(req, res, next){
    let gr_data = req.body;
    let rank_id = parseInt(gr_data.rank_id);
    let guardian_id = parseInt(gr_data.guardian_id);
    let deleteGR = `DELETE FROM Guardian_rank WHERE guardian_id = ? AND rank_id = ?`; 

    db.pool.query(deleteGR, [guardian_id, rank_id], function(error, rows, fields){
        if (error){
            console.log(error);
            res.sendStatus(400);
        }
